...
def getCompaniesDataframe(googleAPIKey,
    ...
    if resource == "https://it.indeed.com/":
        if maxPages > 1:
            ...
            urlList += [
                f"https://it.indeed.com/jobs?q={jobName}&l={locationName}&start={i}0" for i in range(1, maxPages+1)]
        else:
            urlList = [f"https://it.indeed.com/offerte-lavoro?q={jobName}&l={locationName}"]
    ...
    assert len(companyInfo["companyName"]) == len(
        companyInfo["jobName"]) == len(companyInfo["link"])
    ...
    for company in companyInfo["companyName"]:
        ...
        url = 'https://maps.googleapis.com/maps/api/geocode/json?' + \
            'address={}&key={}'.format(rawName, googleCreds.GOOGLE_API_KEY)
        p = {'address': 'Milano'}
    ...
    assert len(companyInfo["companyName"]) == len(
        companyInfo["jobName"]) == len(companyInfo["link"])
    assert len(companyInfo["lt"]) == len(companyInfo["lg"]) == len(
        companyInfo["address"]) == len(companyInfo["link"])
...
def updateMap():
    """
    The function creates a folium map in html format based on the dataframe,
    obtained from score.score function with following layers:

    Vegetation concentration: Amount of green per square meter in each zone of Milan.

    Companies: Headquarters of the companies which offer the job and corresponding
    links to job description.

    Optimal locations based on companies: Mean and median points for a found set
    of companies. The closer an accommodation to median point, the higher score it gets

    Districts: Borders of administrative zones.

    Environmental monitoring stations: Locations of monitoring stations.

    Air Contamination Heatmap: Predicted scaled concentrations based on values
    obtained from monitoring stations. Linear regression with 3-degree polynomial
    features is used.

    Accommodations for rent (apartments): Apartments found at immobiliare.it for
    a given price range. Each item is scored according to given importances.

    Dangerous Zones: Dangerous zones which are better to avoid according
    to several sources

    Predicted Total Score: A score predicted by Random Forest Regressor based
    on scores obtained for Accomodations. Blue is better, red is worse.
    """
    dfCompanies = pd.read_pickle('data/dfCompanies.pkl')
    ...
    milanodistricts = "data/zonedecentramento.geojson"
    ...
    dfGreen = pd.read_pickle("data/dfGreen.pkl")
    mapGreen = folium.Map(
        location=[location.latitude, location.longitude],
        zoom_start=12,
        tiles='cartodbpositron')

    choropleth = folium.Choropleth('data/zonedecentramento.geojson',
    ...
    dfAir = pd.read_pickle("data/dfAirStations.pkl")
    import json
    with open('data/qaria_stazione.geojson') as data_file:
    ...
    dfAccommodationsExpanded = pd.read_pickle("data/dfAccommodationsExpanded.pkl")
    feature_group7 = folium.FeatureGroup(name='Accommodations for rent (apartments)')

    location = geolocator.geocode("Milano Italy")

    for i, row in dfAccommodationsExpanded.iterrows():
        link = "<a href='https://www.immobiliare.it/annunci/" + \
            str(row['id'])+"/"+" target='_blank'" + ">Link</a>"
    ...
    mapGreen.save(f"results/map.html")
    with open('results/map.html', 'r') as file:
        data = file.read()
    data = data.replace(
        '<head>', '<head> <meta http-Equiv="Cache-Control" Content="no-cache" /> <meta http-Equiv="Pragma" Content="no-cache" /> <meta http-Equiv="Expires" Content="0" />')

    with open("results/map.html", "w") as file:
        file.write(data)


if __name__ == '__main__':
    jobName, price_importance, work_importance, danger_importance, air_importance,
    green_importance, minRent, maxRent = [sys.argv[1]]+[float(p) for p in sys.argv[2:]]
    
    score.score(jobName, price_importance, work_importance, danger_importance,
                air_importance, green_importance, minRent=minRent, maxRent=maxRent)
    updateMap()
